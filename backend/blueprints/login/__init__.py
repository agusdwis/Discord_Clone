...
class CreateTokenResource(Resource):
    ...
    def post(self):
        ...
        if qry_user is not None:
            ...
            encoded = ("%s%s" % (args["password"], user_salt)).encode(
                "utf-8"
            )
            ...
            if (
                hash_pass == qry_user.password
                and qry_user.username == args["username"]
            ):
                user_claims_data["is_user"] = True

            print("======", user_claims_data)

            token = create_access_token(
                identity=args["username"], user_claims=user_claims_data
            )
